...
export default function OrdersPage({ isParsing, setIsParsing }: { isParsing?: boolean, setIsParsing?: (isParsing: boolean) => void }) {
  ...
  const filteredEmails = useMemo(() => {
    return emails.filter(email => {
        if (email.category !== 'order') return false;
      ...
      <div className="grid grid-cols-1 md:grid-cols-[clamp(300px,25%,400px)_1fr] h-[calc(100vh-3.5rem)]">
        <div className="border-r flex flex-col">
            <div className="p-4 border-b">
                <h3 className="font-headline text-lg">Orders</h3>
